...
| Spiral Matrix | • Maintained four dynamic pointers (`startingRow`, `endingRow`, `startingCol`, `endingCol`) to represent the boundaries of the matrix and shrink them inward after completing each respective edge traversal. <br> • Iterated sequentially in four directions (left-to-right, top-to-bottom, right-to-left, bottom-to-top) while continuously validating the element tracking condition (`count < total`) to safely handle non-square matrices and avoid duplicate processing. | O(N * M) time <br> O(1) space | Done |
| Rotate Image (90° Clockwise) | • Transformed the matrix by swapping `matrix[i][j]` with `matrix[j][i]` using a nested loop structure where `j` starts from `i + 1`, effectively converting all rows into columns in-place. <br> • Iterated through each row of the transposed matrix and utilized a two-pointer approach (`s` and `e`) to reverse the elements horizontally, completing the 90-degree clockwise rotation without extra space. | O(N²) time <br> O(1) space | Done |
| Count Primes | • Transformed the problem using the Sieve of Eratosthenes algorithm by initializing a boolean array `prime(n + 1, true)` and explicitly setting `prime[0] = prime[1] = false`. <br> • Iterated through the range from `i = 2` up to `n`, incrementing `count++` when `prime[i]` is true, and utilized an inner loop `for(int j = i * 2; j < n; j += i)` to efficiently mark all its composite multiples as false. | O(N log(log N)) time <br> O(N) space | Done |
...
| **Permutation in String** | ### 1. Frequency Mapping<br>Avoid generating permutations via complex swapping by mapping target character counts into two fixed 26-size hash arrays.<br>### 2. Continuous Window Update<br>Slide a window across the main string, dynamically adjusting index counts by incrementing incoming and decrementing outgoing characters for $O(1)$ updates. | $O(N)$ time<br>$O(1)$ space | Done |
| Remove All Adjacent Duplicates | • **LIFO Element Tracking**: Dynamically evaluate characters sequentially using a string as a stack, ensuring only the most recently added elements are active for comparison.<br>• **Instant Collision Popping**: Trigger an immediate `pop_back()` whenever the current character matches `ans.back()`, eliminating cascading duplicates in a single pass. | $O(N)$ time<br>$O(N)$ space | Done |
